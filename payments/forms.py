...
class PaymentForm(forms.ModelForm):
    ...
    class Meta:
        model = Payment
        fields = ['transaction_amount', 'installments', 'payment_method_id', 'email', 'doc_number']
    ...
    def clean_transaction_amount(self):
        ...
        if float(transaction_amount) != float(self.order.get_total_price()):
            raise forms.ValidationError('Transaction amount não bate com o do banco de dados!')
...
class UpdatePaymentForm(forms.Form):
    ...
    def save(self):
        ...
        if cd['action'] == 'payment.updated':
            ...
            payment.order.paid = True if payment_mp['response']['status'] == 'approved' else False
